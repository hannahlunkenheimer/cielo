...
d_intervention$psych <- rowSums(d_intervention[,c(5, 6, 14)]/3) #friends, love, dreams
d_intervention$health <- rowSums(d_intervention[,c(1, 7, 8, 13)]/4) #sick, safe, decide, find
d_intervention$outcome <- rowSums(d_intervention[,c(9, 10, 11, 12)]/4) #job, sports, happy, brave
d_intervention$wellbe <- rowSums(d_intervention[,c(1, 13, 11, 12)]/4) #job, sports, happy, brave
...
# how do i add a legend here!!!!!!!!!!!!!!!!!!!!!!!
g_allinter <- ggplot(d_intervention2, aes(age,totalinter)) + 
  geom_jitter(aes(age, environment), color = "#fdbf6f", width = .2, height = .2, alpha=.4) +
  geom_smooth(aes(age,environment), color = "#fdbf6f", method = "lm", se = FALSE) +
  geom_jitter(aes(age, people), color = "#e2251d", width = .2, height = .2, alpha=.4) +
  geom_smooth(aes(age,people), color = "#e2251d", method = "lm", se = FALSE) + 
  geom_jitter(aes(age, totalinter), color = "#fc9c9c", width = .2, height = .2, alpha=.4) +
  geom_smooth(aes(age,totalinter), color = "#fc9c9c", method = "lm", se = FALSE) +
  theme_minimal() +
  labs(title="Children's responses to intervention frequency questions by domain cluster", x = "Age", y = "Counts", color = "Legend") +
  theme(axis.text.x = element_text(angle=01)) 
...
longdata <- d_intervention2 %>% dplyr::select(starts_with("outcome"), starts_with("health"), starts_with("psych"), starts_with("environment"), starts_with("id_num")) 
...
correlation_matrixbig3 <- round(cor(big3),1)
corrp.matbig3 <- cor_pmat(big3)
ggcorrplot(correlation_matrixbig3, method ="square", 
  hc.order = TRUE,
  outline.color = "white",
  colors = c("#f0436e", "white", "#ff8200")) +
  ggtitle("Correlations: Big Three and Coach Popovich") 

correlation_matrixbig3j <- round(cor(big3j),1)
corrp.matbig3j <- cor_pmat(big3j)
ggcorrplot(correlation_matrixbig3j, method ="square", 
  hc.order = TRUE,
  outline.color = "white",
  colors = c("#f0436e", "white", "#ff8200")) +
  ggtitle("Correlations: Big Three, Murray, and Coach Popovich")
