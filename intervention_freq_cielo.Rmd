...
load2 <- fact2$loadings[,1:2]
plot(load2,type="n") # set up plot
text(load2,labels=names(d_fa1),cex=.7) # add variable names
...
d_intervention <- d_all %>% dplyr::select(starts_with("help_"), contains(c("helop_friends", "age", "gender", "control...6", "relig_attendserv", "relig_tradition")))
...
d_intervention$relig_attendserv <- recode(d_intervention$relig_attendserv, 'Never' = 1, 'Less than once a year'=2, 'A few times a year' = 3, 'A few times a month' = 4, 'A few times a week' = 5, 'Daily' = 6)
d_intervention$relig_tradition <- recode(d_intervention$relig_tradition, 'Not at all important' = 1, 'Minimally important'=2, 'Moderately important' = 3, 'Very important' = 4)
...
g_irc <- ggplot(data = df_irc, aes(x = Var2, y = Freq, fill = Var1)) +
    scale_fill_manual(values=cbPalette, labels=c("1" = "Never", "2" = "Only on special occasions", "3" = "Pretty often", "4" = "Always", "5" = "I don't know")) +
       geom_bar(stat = "identity", width = .5) +
      theme_classic() +
      labs(title="Figure 1. Children's responses to intervention frequency questions", x = "Question", y = "Counts", fill = "Response") +
...
# can add this line back in if we want labels on each of the response blocks
      #geom_text(aes(label = Freq), position = position_stack(vjust = 0.5), color="black", size=1) +
...
# reordering the labels to be environment and people

# fixing the x axis labels
#p_irc <- p_irc + scale_x_discrete(labels=c("exist_germs" = "Germs", "exist_tooth" = "Tooth \n Fairy", "exist_vitamins" = #"Vitamins", "exist_merm" = "Mermaids", "exist_elf" = "Elves", "exist_fairy" = "Fairies", "exist_soul" = "Souls", "exist_god" #= "God", "exist_santa" = "Santa"))

# faceting by gender
#g_irc <- g_irc + facet_wrap(~Var3, ncol = 1) +
#    theme(panel.spacing = unit(1, "lines")) +
#    scale_fill_discrete(labels=c("1" = "Never", "2" = "Only on special occasions", "3" = "Pretty often", "4" = #"Always", "5" = "I don't know"))
#
#g_irc
...
reg4 <- lm(environment ~ age + gender + control + religiosity + age*gender + age*control + gender*control + age*religiosity + gender*religiosity + control*religiosity, data = d_intervention)
summary(reg4)
...
reg4 <- lm(environment ~ age + gender + control + religiosity + age*gender + age*control + gender*control + age*religiosity + gender*religiosity + control*religiosity, data = d_intervention)
summary(reg4)
reg5 <- lm(environment ~ age + gender + control + religiosity + gender*control, data = d_intervention)
summary(reg5)
...
#
######################
## HL NEEDS TO MAKE THIS FIGURE FOR REG 5 CORRECTLY- DON'T THINK I DID IT RIGHT IN PREVIOUS TRIES...
```
...
g_allinter <- ggplot(d_intervention, aes(age,totalinter)) + 
  geom_jitter(color="#999999") +
  geom_smooth(aes(age,environment), color="#E69F00", method="lm",se=FALSE) +
  geom_smooth(aes(age,people), color="#56B4E9", method="lm",se=FALSE) + 
  geom_smooth(aes(age,totalinter), color="#009E73", method="lm",se=FALSE)

g_allinter <- g_allinter +
   labs(title="Figure 2. Predicted probabilities of interventionist frequency concepts across childhood", x = "Age", y = "Intervention frequency score", fill = "Response") +
  theme_minimal()
