...
g_ivrc <- ggplot(data = df_ivrc, aes(x = Var2, y = Freq, fill = Var1)) +
    scale_fill_manual(values=cbPalette2, labels=c("1" = "No", "2" = "Sometimes", "3" = "Yes", "4" = "I don't know")) +
       geom_bar(stat = "identity", width = .5) +
      theme_classic() +
...
reg5 <- lm(interviewscore ~ age + gender + religiosity + control + age*gender + age*religiosity + gender*religiosity + age*control + gender*control + religiosity*control, data = d_interview2)
summary(reg5)
coef(reg5)

# i think this model is the best!
reg6 <- lm(interviewscore ~ age + gender + religiosity + control + religiosity*control, data = d_interview2)
summary(reg6)
anova(reg6)
coef(reg6)
tab_model(reg6)
...
interaction <- plot_model(reg6, type = "pred", colors = c("#999999", "#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2"), terms = c("religiosity", "control"), axis.title = c("Religiosity", "Intervention score"), legend.title = "Perceived control", )
...
#interaction + ggplot2::scale_fill_discrete(name = "Perceived control", labels = c("Low", "1", "2", "3", "4", "5", "High"))
