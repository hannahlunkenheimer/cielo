...
d_intervention <- d_all %>% dplyr::select("age", "gender", "fam_influence", "fam_religiosity", "parent_beliefs", "framework_score", contains(c("pid", "frame_", "control")))
#
...
tab_model(can_famrelig_int_mod)
...
# Create the plot with modified x-axis labels
can_control_fig <- ggplot(d_can_summary, aes(x = frame_can_make_happen_num, y = count, fill = control_cat)) + 
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = wes_palette("Royal2", n = 5)) +
  geom_signif(comparisons = list(c("control", "no control")), map_signif_level = TRUE, textsize = 4,
              vjust = -0.5, tip_length = 0.02) +
  labs(x = "Can God intervene on Earth, yes or no?", y = "Counts", color = "Control",
       title = "Control") +
  theme_minimal() +
...
signif_df <- d_can_proportions %>%
  filter(control_cat %in% c("control", "no control")) %>%
  group_by(frame_can_make_happen_num) %>%
  summarise(p_value = prop.test(count, total_count)$p.value) %>%
...
can_control_fig <- ggplot(d_can_proportions, aes(x = as.factor(frame_can_make_happen_num), y = proportion, fill = control_cat)) + 
  ...
  labs(x = "Can God intervene on Earth, yes or no?", y = "Proportions", color = "Control",
       title = "Control") +
  geom_signif(comparisons = list(c("control", "no control")), y_position = 4.00,
              map_signif_level=TRUE) +  theme_minimal()

can_control_fig

ggsave("can_control_fig.png", can_control_fig, width = 4, height = 4, dpi = 300)
...
tab_model(earth_age_mod, earth_famrelig_mod, earth_pb_mod, earth_control_mod,
  show.est = TRUE,
  show.ci = 0.95,
  show.se = TRUE,
  std.response = TRUE,
...
earth_famrelig_plot <- ggplot(d_earth, 
                   aes(x = age, 
                       y = frame_on_earth_num, 
                       color = as.factor(fam_religiosity))) + 
  geom_jitter(aes(age, frame_on_earth_num), 
              width = .1, height = .1, alpha=.3) +
  geom_smooth(aes(age, frame_on_earth_num), 
              method = "glm", se = FALSE) +
  scale_color_discrete() +
  labs(x = "Age", y = "Does God intervene on Earth, yes or no?", color = "Family\n Religiosity",
       title = "Interaction between Age\n and Family Religiosity") +
  theme_minimal() +
  theme(text = element_text(family = "Times New Roman"), legend.position = "bottom")
...
earth_pb_plot <- ggplot(d_earth, 
  ...
  geom_smooth(aes(age, frame_on_earth_num), 
              method = "glm", se = FALSE) +
  scale_color_discrete() +
  labs(x = "Age", y = "Does God intervene on Earth, yes or no?", color = "Parent\n Intervention\n Beliefs",
       title = "Interaction between Age\n and Parent Intervention Beliefs") +
  theme_minimal() +
  theme(text = element_text(family = "Times New Roman"), legend.position = "bottom")
...
```{r}
# interaction models
people_famrelig_int_mod <- lm(frame_peoples_lives_num ~ age*fam_religiosity, data = d_people)
summary(people_famrelig_int_mod)
AIC(people_famrelig_int_mod)

people_pb_int_mod <- lm(frame_peoples_lives_num ~ age*parent_beliefs + fam_religiosity, data = d_people)
summary(people_pb_int_mod)
AIC(people_pb_int_mod)

tab_model(people_famrelig_int_mod, people_pb_int_mod,
  show.est = TRUE,
  show.ci = 0.95,
  show.se = TRUE,
  std.response = TRUE,
...
#tab_model(personal_age_mod, personal_famrelig_mod, personal_pb_mod, personal_control_mod,
#  show.est = TRUE,
#  show.ci = 0.95,
#  show.se = TRUE,
#  std.response = TRUE,
...
```{r}
# interaction models
personal_famrelig_int_mod <- lm(frame_own_life_num ~ age*fam_religiosity, data = d_personal)
summary(personal_famrelig_int_mod)
AIC(personal_famrelig_int_mod)

personal_pb_int_mod <- lm(frame_own_life_num ~ age*parent_beliefs + fam_religiosity, data = d_personal)
summary(personal_pb_int_mod)
AIC(personal_pb_int_mod)

tab_model(personal_famrelig_int_mod, personal_pb_int_mod, 
  show.est = TRUE,
  show.ci = 0.95,
  show.se = TRUE,
  std.response = TRUE,
