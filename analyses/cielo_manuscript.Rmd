...
d_all$controlcat <- dplyr::recode(d_all$control...6, 'Really Disagree' = 'No control', 'Disagree'= 'No control', 'Kind of Disagree' = 'No control', 'Kind of Agree' = 'Control', 'Agree' = 'Control', 'Really Agree' = 'Control')
...
## correlation between religious tradition and religious service attendance 
d_relig2corr <- d_all %>% dplyr::select(contains(c("relig_tradition", "relig_attendserv")))

d_relig2corr <- na.omit(d_relig2corr)

head(d_relig2corr)

tab_corr(d_relig2corr,
         p.numeric = TRUE,
         triangle = "lower")
...
d_framing <- d_all %>% dplyr::select(contains(c("age", "rgod_happen_earth", "rgod_happen_ever", "rgod_happen_lives", "rpers_happen", "id_num", "controlcat")))
...
d_framingreg <- d_all %>% dplyr::select("control", contains(c("age", "rgod_happen_earth", "rgod_happen_ever", "rgod_happen_lives", "rpers_happen", "id_num", "gender", "famreligiosity", "parentbelief", "school", "interview", "faminfluence", "controlcat")))
...
summary(reg_frame1)
reg_frame7 <- lm(interview ~ age*famreligiosity, data = d_framingreg)
summary(reg_frame7)
...
reg_frame1.5 <- lm(rpers_happen ~ famreligiosity*age, data = d_framingreg)
summary(reg_frame1.5)
...
reg_frame2.6 <- lm(interview ~ parentbelief*age + famreligiosity, data = d_framingreg)
summary(reg_frame2.6)
...
reg_frame3.5 <- lm(interview ~ control*age, data = d_framingreg)
summary(reg_frame3.5)
reg_frame3.6 <- lm(interview ~ control*famreligiosity, data = d_framingreg)
summary(reg_frame3.6)
reg_frame3.7 <- lm(interview ~ control*parentbelief, data = d_framingreg)
...
#creating dataframe for intervention framing fem
d_domains <- d_all %>% dplyr::select("control", contains(c("age", "environment", "social", "psych", "achieve", "id_num", "gender", "famreligiosity", "parentbelief", "health", "alldomains", "faminfluence", "controlcat")))
...
```{r just messing around}
messing1 <- ggplot(d_domains, aes(x = famreligiosity, y = alldomains, color = controlcat)) + 
  geom_jitter() +
  geom_smooth(aes(famreligiosity, alldomains, group = controlcat), method = "glm", se = FALSE)

messing1
```

```{r just messing around}
messing2 <- ggplot(d_domains, aes(x = faminfluence, y = alldomains, color = controlcat)) + 
  geom_jitter() +
  geom_smooth(aes(faminfluence, alldomains, group = controlcat), method = "glm", se = FALSE)

messing2

messing5 <- ggplot(d_domains, aes(x = age, y = alldomains, color = controlcat)) + 
  geom_jitter() +
  geom_smooth(aes(age, alldomains, group = controlcat), method = "glm", se = FALSE) +
  xlim(0, 12) +
  theme_minimal()
...
```{r just messing around}
messing3 <- ggplot(d_framingreg, aes(x = faminfluence, y = interview, color = controlcat)) + 
  geom_jitter() +
  geom_smooth(aes(faminfluence, interview, group = controlcat), method = "glm", se = FALSE) +
  xlim(0, 5.5) +
  theme_minimal()

messing3

messing4 <- ggplot(d_framingreg, aes(x = age, y = interview, color = controlcat)) + 
  geom_jitter() +
  geom_smooth(aes(age, interview, group = controlcat), method = "glm", se = FALSE) +
  xlim(0, 12) +
  theme_minimal()
...
```{r messing with control}
control_mod1 <- lm(interview ~ faminfluence*controlcat, data = d_framingreg)
summary(control_mod1)
control_mod2 <- lm(interview ~ age*controlcat, data = d_framingreg)
summary(control_mod2)

#from what we see here, if we collapse everything into a family influence variable, we see a significant interaction of 

control_mod1 <- lm(alldomains ~ faminfluence*controlcat, data = d_domains)
summary(control_mod1)
control_mod2 <- lm(alldomains ~ age*controlcat, data = d_domains)
summary(control_mod2)
